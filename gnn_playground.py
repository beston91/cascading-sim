...
class GCN(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels):
        super().__init__()
        self.conv1 = GCNConv(
            in_channels, hidden_channels, cached=True, normalize=not args.use_gdc
        )
        self.conv2 = GCNConv(
            hidden_channels, hidden_channels, cached=True, normalize=not args.use_gdc
        )
        self.conv3 = GCNConv(
            hidden_channels, out_channels, cached=True, normalize=not args.use_gdc
        )
...
def test(data):
    ...
    for mask in [data.train_mask, data.val_mask, data.test_mask]:
        accs.append(int((pred[:, None][mask] == data.y[mask]).sum()) / int(mask.sum()))
    return accs
