...
def plot_results(x_data, results, result_type, measures, n, start, step):
    ...
    for index, metric_name in enumerate(measures):
        ...
        error = np.round(
            np.abs(results[:, num_measures + index] - results[:, index]), 2
        )
    ...
    plt.savefig(
        save_dir
        + "approximation_{}_n={},start={},step={}.pdf".format(
            result_type, n, start, step
        )
    )
    plt.show()
...
def main():
    ...
    graph_results = run_analysis(
        n=n, runs=30, k_start=start, k_step=step, measures=measures_graph
    )
