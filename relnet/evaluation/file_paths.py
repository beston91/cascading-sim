...
class FilePaths:
    ...
    def __init__(self, parent_dir, experiment_id, setup_directories=True):
        ...
        self.real_world_graphs_dir = (
            Path(self.parent_dir) / self.REAL_WORLD_GRAPHS_DIR_NAME
        )
    ...
    @staticmethod
    def construct_model_identifier_prefix(
        agent_name,
        obj_fun_name,
        ...
        model_identifier_prefix = (
            f"{agent_name}-{obj_fun_name}-{network_generator_name}-{(graph_id + '-') if graph_id is not None else ''}"
