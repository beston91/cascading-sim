...
class Simulation:
    ...
    def get_graph_coordinates(self):
        ...
        if self.prm["edge_style"] == "bundled":
            pos = (
                pd.DataFrame.from_dict(node_pos, orient="index", columns=["x", "y"])
                .rename_axis("name")
                .reset_index()
            )
    ...
    def plot_results(self, results):
        ...
        if self.child_class() == "Diffusion":
            plt.plot(
                results_norm,
                label="Effective strength: {}".format(self.get_effective_strength()),
            )
            ...
            self.child_class() == "Cascading"
            or self.child_class() == "Attack"
            or self.child_class() == "Defense"
        ):
        ...
        plt.savefig(
            os.path.join(
                self.save_dir, self.get_plot_title(self.prm["steps"]) + "_results.pdf"
            )
        )
        plt.show()
    ...
    def get_plot_title(self, step):
        ...
        if self.child_class() == "Diffusion":
            ...
            title = "Cascading:step={},l={},r={},k_a={},attack={},k_d={},defense={}".format(
                step,
                self.prm["l"],
                self.prm["r"],
                self.prm["k_a"],
    ...
    def get_visual_settings(self, step):
        ...
        if self.child_class() == "Cascading":
            ...
            ns = [
                120 if status == 1 else 40 for status in self.sim_info[step]["status"]
            ]
    ...
    def run_simulation(self):
        ...
        for t in range(self.prm["steps"]):
            avg_results.append(
                np.mean([sim_results[r][t] for r in range(self.prm["runs"])])
            )
