...
class GraphEdgeEnv(object):
    def __init__(self, objective_function, objective_function_kwargs, edge_budget_percentage):
    ...
    def setup(self, g_list, initial_objective_function_values, training=False):
        ...
        self.edge_budgets = self.compute_edge_budgets(
            self.g_list, self.edge_budget_percentage
        )
        ...
        if self.training:
            self.objective_function_values[0, :] = np.multiply(
                self.objective_function_values[0, :], self.reward_scale_multiplier
            )
    ...
    def compute_edge_budgets(g_list, edge_budget_percentage):
        ...
        for i in range(len(g_list)):
            g = g_list[i]
            n = g.num_nodes
            edge_budgets[i] = NetworkGenerator.compute_number_edges(
                n, edge_budget_percentage
            )
            ...
            if not self.exhausted_budgets[i]:
                if actions[i] == -1:
                    if self.logger_instance is not None:
                        ...
                        g = self.g_list[i]
                        self.logger_instance.error(
                            f"first_node selection: {g.first_node}"
                        )

                remaining_budget = self.get_remaining_budget(i)
                self.g_list[i], updated_budget = self.apply_action(
                    self.g_list[i], actions[i], remaining_budget
                )
                ...
                if self.n_steps % 2 == 1:
                    if self.g_list[i].banned_actions == self.g_list[i].all_nodes_set:
                        ...
                        objective_function_value = self.get_objective_function_value(
                            self.g_list[i]
                        )
                        if self.training:
                        ...
                        reward = (
                                ...
                                - self.objective_function_values[0, i]
                        )
