...
class LowestToRandomDegreeAgent(BaselineAgent):
    ...
    def pick_actions_using_strategy(self, t, i):
        ...
        smallest_degree_nodes = [
            n for n in valid_first_nodes if degrees[n] == smallest_degree
        ]

        for first in smallest_degree_nodes:
            budget = self.environment.get_remaining_budget(i)
            valid_choices = list(
                self.environment.get_valid_actions(
                    g, g.get_invalid_edge_ends(first, budget)
                )
            )
...
class LowestDegreeProductAgent(BaselineAgent):
    ...
    def get_local_feature_products(self, g, non_edges):
        degrees = g.node_degrees
        degree_products = list(
            map(lambda pair: degrees[pair[0]] * degrees[pair[1]], non_edges)
        )
...
class LowestBetweennessProductAgent(LowestDegreeProductAgent):
    ...
    def get_local_feature_products(self, g, non_edges):
        ...
        degree_products = list(
            map(lambda pair: betweeness[pair[0]] * betweeness[pair[1]], non_edges)
        )
        return degree_products
...
class FiedlerVectorAgent(BaselineAgent):
    ...
    def pick_actions_using_strategy(self, t, i):
        ...
        abs_differences = list(
            map(
                lambda pair: abs(fiedler_vector[pair[0]] - fiedler_vector[pair[1]]),
                non_edges,
            )
        )
...
class EffectiveResistanceAgent(BaselineAgent):
    ...
    def pick_actions_using_strategy(self, t, i):
        ...
        pairwise_effective_resistances = list(
            map(get_pairwise_effective_resistance, non_edges)
        )
